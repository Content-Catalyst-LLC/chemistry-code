...
func main() {
	dataPath := filepath.Join("..", "data", "nanomaterial_candidates.csv")
	...
	for _, row := range rows[1:] {
		if len(row) != 9 {
			panic("malformed nanomaterial candidate row")
		}
		classes[row[1]] = true
		if row[8] == "true" {
			criticalCount++
	...
	fmt.Println("Go nanochemistry CSV audit")
	fmt.Printf("Candidate rows: %d\n", len(rows)-1)
	fmt.Printf("Nanomaterial class count: %d\n", len(classes))
